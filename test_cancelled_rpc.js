...
async function run() {
  ...
  const { data: order, error: orderErr } = await supabaseAdmin
    .from('orders')
    .insert({ user_id: userId, product_id: productId, amount: 10, payment_method: 'bep20', status: 'cancelled', quantity: 1, wallet_address: '0xtest_cancelled' })
    .select().single();
    
  if (orderErr) { console.error(orderErr); return; }
  
  console.log('Order created as cancelled:', order.id);
