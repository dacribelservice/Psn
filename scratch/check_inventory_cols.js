...
async function checkInventory() {
  console.log('Checking inventory_codes table...');
  const { data, error } = await supabase.from('inventory_codes').select('*').limit(1);
  if (error) {
    console.error('Error inventory_codes:', error);
  } else {
